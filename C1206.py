        ...
        if f'(property "{prop}"' in line:
            if prop == 'Value':
                ...
                line = line.replace('capacitor cap', 
                                    f'C1206 {row["Value"]} {row["Voltage"]} {row["Dielectric"]} {row["Tolerance"]}')
            elif prop == 'ki_description':
                line = line.replace('Unpolarized capacitor, small symbol', 
                                    f'MLCC, {row["Value"]}, {row["Voltage"]}, {row["Dielectric"]}, {row["Tolerance"]}, C1206')
            break
    return line

def write_to_output_file(row, lines):
    # Open output file in append mode
    with open('C1206.kicad_sym', 'a') as f:
        for line in lines:
...
df = pd.read_csv('C1206.CSV')

# Open template file and read its content
with open('template1206.kicad_sym', 'r') as f:
    lines = f.readlines()

# Create output file and write the initial line
with open('C1206.kicad_sym', 'w') as f:
